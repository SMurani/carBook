(function($, window) {
    ...
    slimScrollSettings.height =  windowHeight-172;
    $('#chat .media-list').slimScroll(slimScrollSettings);

    $('#chat a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        if ( e.target.hash == '#chat-settings' )
        {
            slimScrollSettings.height =  windowHeight-108;
            $('#chat #chat-settings').slimScroll(slimScrollSettings);
        }
    });
